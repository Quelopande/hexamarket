...
$istatement = $connection->prepare('SELECT * FROM orders WHERE articleId = :articleId AND purchaserId = :purchaserId LIMIT 1');
$istatement->execute(array(':articleId' => $targetId, ':purchaserId' => $purchaserId));
$iresult = $estatement->fetch();
?>
<!doctype html>
<html lang="en">
<head>
            ...
            <script>
                ...
                if(isset($user)){
                    if($publisherId == $publisherId){
                        echo "<p style='color: #ffffff;text-align:center;font-size: 16px;background: #ff000038;border-radius: 16px;padding: 20px;margin : 10px;'>You can´t purchase for your own resource.</p>";
                    } else{
                        if($iresult == 'completed'){
                            echo '<button type="submit" class="download-btn" data-timer="0" id="download" onclick="window.location.href=`' . $article->file . '`;">Purchased, Download now</button>';
                        } else{
                    ...
                    echo '<button type="submit" class="download-btn" data-timer="0" id="download" onclick="window.location.href=`/login?r=' . $uri . '`;">Buy Now for ' . $article->price . '</button>';
                }
            }
            ?>
        </div>
